...
async function run() {
  intro('StartgptCommit — improving lame commit messages');
  ...
  try {
    if (github.context.eventName === 'push') {
      ...
      outro('Wrong action.');
      core.error(
        `StartgptCommit was called on ${github.context.payload.action}. StartgptCommit is supposed to be used on "push" action.`
      );
    }
  } catch (error: any) {
